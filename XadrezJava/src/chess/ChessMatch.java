...
public class ChessMatch {
    ...
    private Piece makeMove(Position source,Position target){
        ...
        if(p instanceof King && target.getColomn() == source.getColomn() + 2){
            Position sourceT = new Position(source.getRow(), source.getColomn()+3);
            Position targetT = new Position(source.getRow(), source.getColomn()+1);
        ...
        if(p instanceof King && target.getColomn() == source.getColomn() - 2){
            Position sourceT = new Position(source.getRow(), source.getColomn()-4);
            Position targetT = new Position(source.getRow(), source.getColomn()-1);
            ChessPiece rook = (ChessPiece) board.removepiece(sourceT);
            board.placePiece( targetT,rook);
    ...
    public ChessPiece replacePromotedPiece(String q) {
        if(promoted == null){
            throw  new IllegalStateException("Não há peça para ser promovida");
        }
        if(!q.equals("B") && !q.equals("N") && !q.equals("R")&& !q.equals("Q")){
            return promoted;
        }
        ...
        ChessPiece newPiece = newPiece(q, promoted.getColor());
